...
// TODO: fix this function to only give one path
// std::vector<std::vector<int>>
// Solver::computeAllFromStart(const std::vector<std::vector<double>> &D, int s) {
//   const int n = (int)D.size();
//   const int FULL = (1 << n) - 1;
//   const double INF = std::numeric_limits<double>::infinity();
//   auto idx = [n](int mask, int j) { return (size_t)mask * n + j; };
//   std::vector<double> dp((size_t)(1 << n) * n, INF);
//   std::vector<int> parent((size_t)(1 << n) * n, -1);
...
//         continue;
//       int rest = (~mask) & FULL;
//       for (int k = rest; k; k &= (k - 1)) {
//         int v = __builtin_ctz(k);
//         double cand = cur + D[j][v];
//         int nmask = mask | (1 << v);
//         size_t ii = idx(nmask, v);
//         if (cand < dp[ii]) {
//           dp[ii] = cand;
//           parent[ii] = j;
//         }
...
// std::vector<int>
// Solver::optimal_tour(const std::vector<std::vector<double>> &D) {
//   const int n = (int)D.size();
//   if (n == 0)
//     return {};

//   const int start = 0;
//   auto res = computeAllFromStart(D, start);

//   // Sum length of a path using D
//   auto path_cost = [&](const std::vector<int> &path) -> double {
//     if (path.empty())
...
//   // Find the best endpoint (last city before returning to start)
//   for (int t = 0; t < n; ++t) {
//     if (t == start)
//       continue;
//     const auto &path = res[t];
//     if (path.empty() || path.front() != start)
//       continue; // unreachable
//     double total = path_cost(path) + D[t][start];
//     if (total < best) {
//       best = total;
//       best_end = t;
//     }
...
//   std::vector<int> tour = res[best_end];
