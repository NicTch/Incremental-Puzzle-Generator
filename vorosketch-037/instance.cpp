...
// std::vector<Candidate> Instance::uniformPicker(int n) {
...
// void Instance::keepBestAndRefill(std::vector<Candidate> &cs, Instance &ctx,
//                                  int width, Options &opt) {
//   if (cs.empty())
//     return;

//   auto it = std::max_element(
//       cs.begin(), cs.end(), [&ctx](const Candidate &a, const Candidate &b) {
//         return edgeChangeOf(ctx, a) < edgeChangeOf(ctx, b);
//       });
//   Candidate best = *it;

//   std::vector<Candidate> pool = ctx.initCandidate(opt);
//   pool.erase(std::remove_if(
//                  pool.begin(), pool.end(),
//                  [&](const Candidate &c) { return sameCandidate(c, best); }),
//              pool.end());

//   std::mt19937 gen(1337);
//   std::shuffle(pool.begin(), pool.end(), gen);

//   // target width (1 best + up to width-1 fills)
//   const size_t target = (width > 0 ? (size_t)width : cs.size());
...
//   // if we still have room, top-up from old cs (excluding best), shuffled
//   if (out.size() < target) {
//     std::vector<Candidate> oldTail;
//     oldTail.reserve(cs.size());
//     for (const auto &c : cs)
//       if (!sameCandidate(c, best))
//         oldTail.push_back(c);
//     std::shuffle(oldTail.begin(), oldTail.end(), gen);
//     for (const auto &c : oldTail) {
//       if (out.size() == target)
//         break;
//       bool dup = false;
//       for (const auto &x : out) {
//         if (sameCandidate(x, c)) {
//           dup = true;
//           break;
//         }
//       }
...
//   cs.swap(out);
// }
// Rebuild `node->children` so they match `node->candidates` (1:1, same
// order).
// static void rebuildChildrenForNode(tsp_puzzle::Instance *node, Options &opt)
// {
//   using tsp_puzzle::Candidate;
//   using tsp_puzzle::Instance;
//
//   std::vector<std::unique_ptr<Instance>> newChildren;
//   newChildren.reserve(node->candidates.size());
//
//   // For each desired candidate, either reuse an existing child or create a
//   new
//   // one.
//   for (const Candidate &cand : node->candidates) {
//     // Try to find a matching existing child
//     int found = -1;
//     for (int i = 0; i < (int)node->children.size(); ++i) {
//       if (sameCandidate(node->children[i]->fromCandidate, cand)) {
//         found = i;
//         break;
//       }
//     }
//
//     if (found >= 0) {
//       // Reuse: move the child into the new vector
//       newChildren.push_back(std::move(node->children[found]));
//     } else {
//       // Create a new child instance for this candidate
//       std::vector<tsp_puzzle::Node> childNodes = node->nodes;
//       tsp_puzzle::Node added;
//       added.x = cand.x;
//       added.y = cand.y;
//       added.idx = (int)childNodes.size();
//       childNodes.push_back(added);
//
//       auto child =
//           std::make_unique<Instance>(childNodes, opt); // recomputes
//           decs+cands
//       child->parent = node;
//       child->fromCandidate = cand;
//       newChildren.push_back(std::move(child));
//     }
//   }
//   // Any leftover (unmoved) children get destroyed here when we overwrite the
//   // vector.
//   node->children = std::move(newChildren);
// }
// void Instance::refreshSubtreeCandidates(Instance *node, int branching,
//                                         Options &opt) {
//   if (!node)
//     return;
//
//   keepBestAndRefill(node->candidates, *node, branching, opt);
//
//   if (branching > 0 && (int)node->candidates.size() > branching) {
//     std::mt19937 gen(1337);
//     std::shuffle(node->candidates.begin(), node->candidates.end(), gen);
//     node->candidates.resize(branching);
//   }
//   rebuildChildrenForNode(node, opt);
//   for (auto &ch : node->children)
//     refreshSubtreeCandidates(ch.get(), branching, opt);
// }
